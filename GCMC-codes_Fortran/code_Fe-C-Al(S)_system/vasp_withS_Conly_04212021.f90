      !Primary verison of 20210323 for VASP on coking formation
      !New feature for having one element other than Fe or C
      ...
      !First line of POSCAR must have " C". So that Ccount is the indicator of whether there is C in the system (if Ccount>1)

      !S, Fe, and C are considered in the system (but just Fe/C atoms as MC_atoms) in current version (in read and mag, 1-S, 2-Fe, 3-C)
      !Only the use of relaxation (but not AIMD) is considered in current version
      !Failed stuctures were logged
      ...
      integer :: nmciter, iRX, imc, nfail, types, iter, &
         & ndumb, natoms, natoms1, natoms2, natoms3, natoms1_System, natoms2_System, natoms3_System, natoms_System, niterMC, nPermanentAtoms_System, &
         ...
         & imagetest_z, idebug, iter_cmc, type_check, natoms_trial, natoms1_trial, natoms2_trial, natoms3_trial, iz_place, Ccount
      logical, save :: debug
...
!arrange files in directory and run initial VASP on System 
      ...
      call ReadCoordinates(emc,cmc,tfmc,natoms1,natoms2,natoms3,natoms,xaxismc,yaxismc,zaxismc)   !!start from a calculation result directly
...
! (fail-log)       
      ...
      write (15,'(4i7,f14.8)')nfail,imc,nMCatoms_System(:), emc 
      close (15)
...
!*****************************************
     ...
     if ((iMCflag.eq.0).and.((natoms_System-nPermanentAtoms_System).gt.0).and.(disp_prob.gt.0)) then     !!half of displacements are accepted                       
         ...
         if (nMCatomMoved.gt.(natoms1_System + natoms2_System)) then                   !determine which atom type was selectedn
               iAtom = 2     !! C               
         else
               iAtom = 1     !! Fe
         end if         
...
call GetRandomCoords(natoms_System,cmc_System,Rmin,Rmax,xaxismc,yaxismc,zaxismc,nmcmax,randCoord,iz_place,zmin,zmax,iAtom, ipass)   !select new coordinates (Rmin checked in subroutine)     
     ...
     if (ipass.eq.1) then 
         ...
         natoms3_trial = natoms3_System
         natoms_trial = natoms_System
         
         !Run VASP
         !**********
         call Writecoordinates(xaxismc,yaxismc,zaxismc,cmc_trial,tfmc_trial,natoms1_trial,natoms2_trial,natoms3_trial,natoms_trial)
         call Writemagmom(natoms1_trial, natoms2_trial, natoms3_trial)
         call RunVasp(natoms_trial) ! run Vasp with new geo
         call ReadCoordinates(emc,cmc,tfmc,natoms1,natoms2,natoms3,natoms,xaxismc,yaxismc,zaxismc) !read in new Reax data
...
!*     Remove Atom Block
!********************************************
      elseif ( (iMCflag.eq.1).and.((natoms_System-nPermanentAtoms_System).gt.1) ) then     !skip if no exchangeable atoms left to remove
         ...
         if (nMCatomRemoved.gt.(natoms1_System + natoms2_System)) then                   !determine which atom type was selected for deletion
               natoms3_trial = natoms3_System -1
               natoms2_trial = natoms2_System
               natoms1_trial = natoms1_System
               iAtom = 2                    
         else
     ...
     if (ipass.eq.1) then           
         ...
         if (iAtom.eq.1) then     !!Add Fe
               ...
               if (natoms2_System.ne.0) then
                    do i4 = natoms1_trial+natoms2_trial+1,natoms_trial
                         cmc_trial(i4,:) = cmc_System(i4-1,:)
                         tfmc_trial(i4,:) = tfmc_System(i4-1,:)
                    end do
                    
                    cmc_trial(natoms1_trial+natoms2_trial,:) = randCoord(:) ! add random MC atom coords after Fe atoms
                    tfmc_trial(natoms1_trial+natoms2_trial,1) = 'T'
                    tfmc_trial(natoms1_trial+natoms2_trial,2) = 'T'
                    tfmc_trial(natoms1_trial+natoms2_trial,3) = 'T'
                    
               elseif (natoms3_System.eq.0) then
                    cmc_trial(natoms1_trial+natoms2_trial,:) = randCoord(:) ! add random MC atom coords to the end
                    tfmc_trial(natoms1_trial+natoms2_trial,1) = 'T'
                    tfmc_trial(natoms1_trial+natoms2_trial,2) = 'T'
                    tfmc_trial(natoms1_trial+natoms2_trial,3) = 'T'
...
!**********************************************************************
      ...
      distmin = 1000.0 !set high distmin value to ensure while loop is enetered
         ...
         distmin = 1000.0                  !set arbitrarily high distmin value to reset distmin after each while loop iteration
             ...
             do imagetest_y=-1,1
                 do imagetest_z=-1,1 
                     do icheck=1,natoms       !calculate distance to all atoms in the system 
                         ...
                         dist(1) = abs((cmc(icheck,1)+imagetest_x-randCoord(1))*xaxismc(1) &
& + (cmc(icheck,2)+imagetest_y-randCoord(2))*yaxismc(1) + (cmc(icheck,3)+imagetest_z-randCoord(3))*zaxismc(1))    !!! Updated for non-orthogonal cells      
                         dist(2) = abs((cmc(icheck,2)+imagetest_y-randCoord(2))*yaxismc(2) &
& + (cmc(icheck,1)+imagetest_x-randCoord(1))*xaxismc(2) + (cmc(icheck,3)+imagetest_z-randCoord(3))*zaxismc(2))
                         dist(3) = abs((cmc(icheck,3)+imagetest_z-randCoord(3))*zaxismc(3) &
& + (cmc(icheck,1)+imagetest_x-randCoord(1))*xaxismc(3) + (cmc(icheck,2)+imagetest_y-randCoord(2))*yaxismc(3))
...
subroutine ReadCoordinates(emc,cmc,tfmc,natoms1,natoms2,natoms3,natoms,xaxismc,yaxismc,zaxismc)
! reads in 'CONTCAR' data
! Use Ccount from CONTCAR to determine atom types
!********************************************************************** 
      implicit none
      integer, parameter :: KREAL = kind(0.d0)
      integer :: natoms, natoms1, natoms2, natoms3, i1, Ccount
      ...
      character*2 :: qatom(3), qdumb1, qdumb2, qdumb3, qdumb4
      ...
      if ( Ccount > 1) then    
          read (97,*)qatom(1), qatom(2), qatom(3)   !! 1-S, 2-Fe, 3-C
          read (97,*)natoms1, natoms2, natoms3
      else
...
!********************************************************************** 
    ...
    subroutine Writecoordinates(xaxismc,yaxismc,zaxismc,cmc,tfmc,natoms1,natoms2,natoms3,natoms)
! writes geometry data in POSCAR format
! Use natoms2 to determine atom types
!********************************************************************** 
      implicit none
      integer, parameter :: KREAL = kind(0.d0)
      integer :: natoms, natoms1, natoms2, natoms3, i1
      integer,parameter :: nmcmax=10000
      ...
      if ( natoms3 > 0) then                           !!use natoms3 to determine if there is C, since C number might change before write coordinates 
          write (98,'(3a6)')"S","Fe","C"
          write (98,'(3i6)')natoms1, natoms2, natoms3
      else
          write (98,'(2a6)')"S","Fe"
...
!**********************************************************************
    ...
    subroutine Writemagmom(natoms1, natoms2, natoms3)
...
!********************************************************************** 
      ...
      implicit none
      integer, parameter :: KREAL = kind(0.d0)
      integer :: natoms1, natoms2, natoms3    !1 refer to S(Al), 2 refer to Fe, 3-C
      real(KREAL) :: magC, magFe, magNFe, magS, magAl
      ...
      if (natoms3 > 0) then
           open (76,file='tmpmagmom',access='append')
               write (76,'(a9,i4,a1,f4.2,i4,a1,f4.2,i4,a1,f5.2,i4,a1,f4.2)') "MAGMOM = ", natoms1, "*", magS, natoms2/2, "*", magFe, natoms2/2, "*", magNFe, natoms3, "*", magC 
           close (76)
      else
           open (76,file='tmpmagmom',access='append')
               write (76,'(a9,i4,a1,f4.2,i4,a1,f4.2,i4,a1,f5.2)') "MAGMOM  = ", natoms1, "*", magS, natoms2/2, "*", magFe, natoms2/2, "*", magNFe
           close (76)
      end if
